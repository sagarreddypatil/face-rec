...
import PIL.ImageOps
import os
from async_cap import AsyncCap
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if (
            first_eye_center == (0, 0)
            or second_eye_center == (0, 0)
            or first_eye_center == second_eye_center
        ):
            pass
        else:
            try:
                ...
                cv2.line(
                    face_color, first_eye_center, second_eye_center, (255, 0, 0), 1
                )
                deg = int(
                    atan(
                        (first_eye_center[1] - second_eye_center[1])
                        / (first_eye_center[0] - second_eye_center[0])
                    )
                    * 180
                    / pi
                )
        ...
        # print("-------------------------------")
        # print("probabilities: " + str(pred_prob))
        # print("id: " + str(pred_id))
        # print("prob for id: " + str((pred_prob[0][pred_id]) * 100))
        # print("label: " + str(pred_label))
        # print("-------------------------------")
        # font = cv2.FONT_HERSHEY_SIMPLEX
        # cv2.putText(img, pred_label + " " + str((pred_prob[0][pred_id]) * 100), top_left_new, font, 1, (255, 0, 0), 1, cv2.LINE_AA)
    cv2.imshow("face", img)
